...
export default function OtpScreen({navigation, route}: Props) {
  ...
  const handleVerify = async () => {
    const code = otp.join('');
    if (code.length !== 6) {setError(ar.validation.invalidOtp); return;}
    setLoading(true);
    setError('');
    try {
      ...
      <View style={styles.otpRow}>
        {otp.map((d, i) => (
          <TextInput key={i} ref={r => {if (r) inputs.current[i] = r;}}
            style={styles.otpInput} value={d} onChangeText={v => handleChange(v, i)}
...
const styles = StyleSheet.create({
  ...
  title: {fontSize: 22, fontWeight: 'bold', color: '#00695c', marginBottom: 8, fontFamily: 'Cairo-Bold'},
  ...
  otpInput: {width: 48, height: 56, borderWidth: 2, borderColor: '#00695c', borderRadius: 10, fontSize: 22, fontWeight: 'bold', backgroundColor: '#fff'},
  btn: {backgroundColor: '#00695c', borderRadius: 12, paddingVertical: 14, paddingHorizontal: 60},
